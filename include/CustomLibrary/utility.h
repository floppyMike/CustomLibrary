...
namespace ctl
{
	...
	constexpr T mapVal(const T &val, const T &oldMin, const T &oldMax, const T &newMin, const T &newMax)
	{
		return newMin + (newMax - newMin) * val / (oldMax - oldMin);
	}

	template<typename T, typename = typename std::enable_if_t<std::is_floating_point_v<T>>>
	constexpr T to_degrees(const T &val) noexcept
	{
		return val / PI * 180;
	}
	template<typename T, typename = typename std::enable_if_t<std::is_floating_point_v<T>>>
	constexpr T to_radians(const T &val) noexcept
	{
		return val / 180 * PI;
	}

	template<class CharT, class Traits, class Allocator>
	bool is_number(const std::basic_string<CharT, Traits, Allocator> &str)
	{
		return !str.empty() && str.find_first_not_of("0123456789.") == std::string::npos;
	}
	// bool is_number(const char& ch) { constexpr std::string_view nums = "0123456789."; return std::find(nums.begin(),
	// nums.end(), ch) != nums.end(); }
	...
	auto toBinary(const Type &num)
	{
		return std::bitset<sizeof(Type) * 8>(num);
	}

	template<typename T, typename = typename std::enable_if_t<std::is_arithmetic_v<T>>>
	constexpr auto power2(const T &val) noexcept
	{
		return val * val;
	}
	...
	std::string toHexadecimal(int val)
	{
		...
		if (val == 0) return std::string(1, '0');
	...
	auto multi_static_cast(const Args &... args)
	{
