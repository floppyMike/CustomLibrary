...
namespace ctl::sdl
{
	namespace detail
	{
		...
		class _Option_<Impl, tag::isRenderer>
			: public crtp<Impl, _Option_, tag::isRenderer>
		{
		public:
		...
		template<typename Impl>
		class _Option_<Impl, tag::isFont>
			: public crtp<Impl, _Option_, tag::isFont>
		{
		public:
			void style(int style)
			{
				TTF_SetFontStyle(this->underlying()->obj()->font(), style);
			}

			auto style()
			{
				return TTF_GetFontStyle(this->underlying()->obj()->font());
			}
